/*
  ************************************************************************
  ...
  Name         : 18_rdlck.c
  Author       : Pankaj Deopa
  Description  : Program to implement record locking - Read lock
  Date         : 26 Aug, 2025
...
int main () {
    ...
    if ((fd = open("records.dat",O_RDONLY )) == -1) {
        perror("open");
        exit(1);
    }

    printf("Enter the record number to read [0, 1 or 2] : ");
    scanf("%d", &record_no);
    ...
    // acquire read lock
    printf("Trying to acquire the read lock on record number : %d\n", record_no);
